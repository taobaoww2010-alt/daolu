...
function DiffViewer(props: { api: TuiPluginApi }) {
  ...
  const commands = [
    ...
    {
      name: "diff.down",
      title: zh("Move diff viewer down"),
      category: "VCS",
      run: focusRunner({
        files() {
          moveFileSelection(1)
    ...
    {
      name: "diff.page.down",
      title: zh("Page diff viewer down"),
      category: "VCS",
      run: focusRunner({
        files() {
          moveFileSelection(8)
    ...
    {
      name: "diff.toggle",
      title: zh("Toggle diff viewer item"),
      category: "VCS",
      run: focusRunner({
        files() {
    ...
    {
      name: "diff.expand",
      title: zh("Expand diff viewer item"),
      category: "VCS",
      run: focusRunner({
        files() {
          const highlighted = highlightedFileNode()
    ...
    {
      name: "diff.collapse",
      title: zh("Collapse diff viewer item"),
      category: "VCS",
      run: focusRunner({
        files() {
          const highlighted = highlightedFileNode()
    ...
    {
      name: "diff.next_hunk",
      title: zh("Jump to next diff hunk"),
      category: "VCS",
      run() {
        jumpRelativeHunk(1)
      },
    },
    {
      ...
      title: zh("Jump to previous diff hunk"),
      category: "VCS",
      run() {
        jumpRelativeHunk(-1)
      },
    ...
    {
      name: "diff.next_file",
      title: zh("Jump to next diff file"),
      category: "VCS",
      run() {
    ...
    },
    {
      ...
      title: zh("Jump to previous diff file"),
      category: "VCS",
      run() {
        jumpRelativePatchFile(-1)
      },
    },
    {
      ...
      title: zh("Toggle selected diff file reviewed"),
      category: "VCS",
      run() {
    ...
    },
    {
      ...
      title: zh("Switch diff viewer focus"),
      category: "VCS",
      run() {
    ...
    },
    {
      ...
      title: zh("Toggle diff viewer file tree"),
      category: "VCS",
      run() {
    ...
    },
    {
      ...
      title: zh("Toggle single patch view"),
      category: "VCS",
      run() {
        setSelectedHunk(undefined)
        if (!singlePatch()) {
    ...
    },
    {
      ...
      title: zh("Switch diff viewer source"),
      category: "VCS",
      run() {
    ...
    },
    {
      ...
      title: zh("Toggle diff viewer split or unified view"),
      category: "VCS",
      run() {
    ...
    {
      name: "diff.help",
      title: zh("Show more diff viewer shortcuts"),
      category: "VCS",
      run() {
        openHelpDialog()
      },
  ...
  const switchDiffOptions = createMemo(() => {
    ...
    return [
      {
        title: zh("Working tree"),
        value: "git" as const,
        description: zh("Show current git changes"),
      },
      ...(vcs?.branch && vcs.default_branch && vcs.branch !== vcs.default_branch
        ? [
            {
              title: zh("Main branch"),
              value: "branch" as const,
              description: zh("Show changes compared to main branch"),
            },
          ]
        : []),
      {
        ...
        description: zh("Show changes from the last assistant turn"),
      },
    ]
  })
  ...
  useBindings(() => ({
    commands,
    bindings: [
      { key: "j,down", cmd: "diff.down", desc: zh("Move diff viewer down") },
      { key: "k,up", cmd: "diff.up", desc: zh("Move diff viewer up") },
      { key: "pagedown,ctrl+f", cmd: "diff.page.down", desc: zh("Page diff viewer down") },
      { key: "pageup,ctrl+b", cmd: "diff.page.up", desc: zh("Page diff viewer up") },
      { key: "m", cmd: "diff.mark_reviewed", desc: zh("Mark reviewed") },
      ...
      <PanelGroup axis="y" width="100%" height="100%">
          ...
          <Switch>
            <Match when={diff.loading}>
              ...
              <box flexGrow={1} paddingLeft={1}>
                <text fg={theme().textMuted}>{zh("Loading diff...")}</text>
              </box>
            </Match>
            <Match when={!diff.loading && files().length === 0}>
              <Separator axis="x" />
              <box flexGrow={1} paddingLeft={1}>
                <text fg={theme().textMuted}>{zh("No diff!")}</text>
              </box>
            </Match>
            <Match when={!diff.loading && diff.error}>
              <Separator axis="x" />
              <box flexGrow={1} paddingLeft={1}>
                <text fg={theme().error}>{zh("Failed to load diff")}</text>
              </box>
            </Match>
            <Match when={!diff.loading}>
              <PanelGroup axis="x">
                        ...
                        return (
                          <box ref={(element: BoxRenderable) => registerPatchNode(entry.fileIndex, element)}>
                            ...
                            <Show
                              ...
                              fallback={<text fg={theme().textMuted}>{zh("No patch available for this file.")}</text>}
                            >
                              {(patch) => (
        ...
        <Panel flexShrink={0} gap={2} paddingLeft={1} border="none">
          <Show when={switchFocusShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{zh("focus file tree")}</span>
              </text>
            )}
          </Show>
          <Show when={nextFileShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{zh("next file")}</span>
              </text>
            )}
          </Show>
          <Show when={nextHunkShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{zh("next hunk")}</span>
              </text>
            )}
          </Show>
          <Show when={previousHunkShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{zh("previous hunk")}</span>
              </text>
            )}
          </Show>
          <Show when={previousFileShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{zh("previous file")}</span>
              </text>
            )}
          </Show>
          <Show when={switchSourceShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{zh("switch source")}</span>
              </text>
            )}
          </Show>
          <Show when={markReviewedShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{zh("Mark reviewed")}</span>
              </text>
            )}
          </Show>
          <Show when={helpShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{zh("all")}</span>
              </text>
            )}
          </Show>
        </Panel>
...
function DiffViewerHelpDialog() {
  ...
  const rows = [
    {
      shortcut: () => "q",
      action: zh("Close viewer"),
      description: zh("Quit the diff viewer"),
    },
    {
      shortcut: useCommandShortcut("diff.switch_focus"),
      action: zh("Focus file tree"),
      description: zh("Move keyboard focus between the file tree and patch pane"),
    },
    {
      shortcut: useCommandShortcut("diff.next_hunk"),
      action: zh("Next hunk"),
      description: zh("Jump to the next diff hunk"),
    },
    {
      shortcut: useCommandShortcut("diff.previous_hunk"),
      action: zh("Previous hunk"),
      description: zh("Jump to the previous diff hunk"),
    },
    {
      shortcut: useCommandShortcut("diff.next_file"),
      action: zh("Next file"),
      description: zh("Select the next changed file in file-tree order"),
    },
    {
      shortcut: useCommandShortcut("diff.previous_file"),
      action: zh("Previous file"),
      description: zh("Select the previous changed file in file-tree order"),
    },
    {
      shortcut: useCommandShortcut("diff.toggle_file_tree"),
      action: zh("Toggle file tree"),
      description: zh("Show or hide the file tree sidebar"),
    },
    {
      shortcut: useCommandShortcut("diff.single_patch"),
      action: zh("Toggle patches"),
      description: zh("Switch between one selected patch and all patches"),
    },
    {
      shortcut: useCommandShortcut("diff.switch_source"),
      action: zh("Switch source"),
      description: zh("Choose working tree, main branch, or last-turn changes"),
    },
    {
      shortcut: useCommandShortcut("diff.toggle_view"),
      action: zh("Toggle view"),
      description: zh("Switch between split and unified diff layout"),
    },
    {
      shortcut: useCommandShortcut("diff.expand_all"),
      action: zh("Expand all folders"),
      description: zh("Open every folder in the file tree"),
    },
    {
      shortcut: useCommandShortcut("diff.mark_reviewed"),
      action: zh("Mark reviewed"),
      description: zh("Toggle reviewed state for the selected file"),
    },
  ]
  ...
  return (
    <box paddingLeft={2} paddingRight={2} paddingBottom={1} gap={1}>
      ...
      <box flexDirection="row">
        ...
        <text fg={theme.textMuted}>{zh("Description")}</text>
      </box>
      <For each={rows}>
        {(row) => (
          <box flexDirection="row">
