    ...
    >
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        <Show when={!single()}>
          <box flexDirection="row" gap={1} paddingLeft={1}>
            ...
            >
              <text fg={confirm() ? selectedForeground(theme, theme.accent) : theme.textMuted}>{zh("Confirm")}</text>
            </box>
          </box>
        </Show>
        ...
        <Show when={!confirm()}>
          <box paddingLeft={1} gap={1}>
            <box>
              <text fg={theme.text}>
                {question()?.question}
                {multi() ? ` ${zh("(select all that apply)")}` : ""}
              </text>
            </box>
            <box>
              <For each={options()}>
                  ...
                  <box flexDirection="row">
                    ...
                    <box backgroundColor={other() ? theme.backgroundElement : undefined}>
                      <text fg={other() ? theme.secondary : customPicked() ? theme.success : theme.text}>
                        {multi() ? `[${customPicked() ? "✓" : " "}] ${zh("Type your own answer")}` : zh("Type your own answer")}
                      </text>
                    </box>
        ...
        <Show when={confirm() && !single()}>
          <box paddingLeft={1}>
            <text fg={theme.text}>{zh("Review")}</text>
          </box>
          <For each={questions()}>
            {(q, index) => {
      ...
      >
        <box flexDirection="row" gap={2}>
          <Show when={!single()}>
            <text fg={theme.text}>
              {"⇆"} <span style={{ fg: theme.textMuted }}>{zh("tab")}</span>
            </text>
          </Show>
          <Show when={!confirm()}>
            <text fg={theme.text}>
          ...
          <text fg={theme.text}>
            ...
            <span style={{ fg: theme.textMuted }}>
              {confirm() ? zh("submit") : multi() ? zh("toggle") : single() ? zh("submit") : zh("confirm")}
            </span>
          </text>

          <text fg={theme.text}>
            esc <span style={{ fg: theme.textMuted }}>{zh("dismiss")}</span>
          </text>
        </box>
      </box>
    </box>
