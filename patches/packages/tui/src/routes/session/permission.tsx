...
function EditBody(props: { request: PermissionRequest }) {
  ...
  return (
    <box flexDirection="column" gap={1}>
      ...
      <Show when={!diff()}>
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>{zh("No diff provided")}</text>
        </box>
      </Show>
    </box>
  )
  ...
  return (
    <Switch>
      <Match when={store.stage === "always"}>
        <Prompt
          title={zh("Always allow")}
          body={
            <Switch>
              <Match when={props.request.always.length === 1 && props.request.always[0] === "*"}>
                <TextBody title={`${zh("This will allow")} ${props.request.permission} ${zh("until OpenCode is restarted.")}`} />
              </Match>
              <Match when={true}>
                <box paddingLeft={1} gap={1}>
                  <text fg={theme.textMuted}>{zh("This will allow the following patterns until OpenCode is restarted")}</text>
                  <box>
          ...
          options={{ confirm: zh("Confirm"), cancel: zh("Cancel") }}
          escapeKey="cancel"
          onSelect={(option) => {
        ...
        {(() => {
          const info = () => {
            ...
            if (permission === "doom_loop") {
              return {
                icon: "⟳",
                title: zh("Continue after repeated failures"),
                body: (
                  <box paddingLeft={1}>
                    <text fg={theme.textMuted}>{zh("This keeps the session running despite repeated failures.")}</text>
                  </box>
                ),
              }
            }
          ...
          const header = () => (
            <box flexDirection="column" gap={0}>
              <box flexDirection="row" gap={1} flexShrink={0}>
                ...
                <text fg={theme.text}>{zh("Permission required")}</text>
              </box>
          ...
          const body = (
            <Prompt
              title={zh("Permission required")}
              header={header()}
              body={current.body}
              options={{ once: zh("Allow once"), always: zh("Always allow"), reject: zh("Reject") }}
              escapeKey="reject"
              fullscreen
              onSelect={(option) => {
                if (option === "always") {
    ...
    >
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        <box flexDirection="row" gap={1} paddingLeft={1}>
          ...
          <text fg={theme.text}>{zh("Reject permission")}</text>
        </box>
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>{zh("Tell OpenCode what to do differently")}</text>
        </box>
      </box>
      <box
      ...
      >
        ...
        <box flexDirection="row" gap={2} flexShrink={0}>
          <text fg={theme.text}>
            enter <span style={{ fg: theme.textMuted }}>{zh("confirm")}</span>
          </text>
          <text fg={theme.text}>
            esc <span style={{ fg: theme.textMuted }}>{zh("cancel")}</span>
          </text>
        </box>
      </box>
    </box>
...
function Prompt<const T extends Record<string, string>>(props: {
  ...
  useBindings(() => ({
    ...
    commands: [
      ...
      },
      {
        name: "permission.prompt.fullscreen",
          title: zh("Toggle permission fullscreen"),
        category: "Permission",
        run() {
    ...
    const hint = createMemo(() => (store.expanded ? zh("minimize") : zh("fullscreen")))
  useRenderer()
    ...
    >
      ...
      >
        ...
        <box flexDirection="row" gap={2} flexShrink={0}>
          ...
          <text fg={theme.text}>
            {"⇆"} <span style={{ fg: theme.textMuted }}>{zh("select")}</span>
          </text>
          <text fg={theme.text}>
            enter <span style={{ fg: theme.textMuted }}>{zh("confirm")}</span>
          </text>
