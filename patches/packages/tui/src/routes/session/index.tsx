...
export function Session() {
  ...
  const sessionCommandList = createMemo(() => [
    {
      title: session()?.share?.url ? zh("Copy share link") : zh("Share session"),
      value: "session.share",
      ...
      run: async () => {
        const copy = (url: string) =>
          clipboard
            .write?.(url)
            .then(() => toast.show({ message: zh("Share URL copied to clipboard!"), variant: "success" }))
            .catch(() => toast.show({ message: zh("Failed to copy URL to clipboard"), variant: "error" }))
        ...
        if (!kv.get("share_consent", false)) {
          const ok = await DialogConfirm.show(dialog, zh("Share Session"), zh("Are you sure you want to share it?"))
          if (ok !== true) return
        ...
        await sdk.client.session
          ...
          .catch((error) => {
            toast.show({
              message: error instanceof Error ? error.message : zh("Failed to share session"),
              variant: "error",
            })
          })
        dialog.clear()
    ...
    },
    {
      ...
      run: () => {
        ...
        if (!selectedModel) {
          toast.show({
            variant: "warning",
            message: zh("Connect a provider to summarize this session"),
            duration: 3000,
          })
          return
        }
    ...
    },
    {
      ...
      run: async () => {
        await sdk.client.session
          ...
          .then(() => toast.show({ message: zh("Session unshared successfully"), variant: "success" }))
          .catch((error) => {
            toast.show({
              message: error instanceof Error ? error.message : zh("Failed to unshare session"),
              variant: "error",
            })
          })
        dialog.clear()
    ...
    {
      title: zh("Undo previous message"),
      value: "session.undo",
      category: "Session",
      slash: {
        name: "undo",
    ...
    {
      title: sidebarVisible() ? zh("Hide sidebar") : zh("Show sidebar"),
    ...
    {
      title: conceal() ? zh("Disable code concealment") : zh("Enable code concealment"),
    ...
    {
      title: showTimestamps() ? zh("Hide timestamps") : zh("Show timestamps"),
    ...
    },
    {
      title: (() => {
        ...
        if (next === "hide") return zh("Collapse thinking")
        return zh("Expand thinking")
      })(),
    ...
    {
      title: showDetails() ? zh("Hide tool details") : zh("Show tool details"),
    ...
    {
      title: zh("Toggle session scrollbar"),
    ...
    {
      title: showGenericToolOutput() ? zh("Hide generic tool output") : zh("Show generic tool output"),
    ...
    {
      title: zh("Jump to last user message"),
    ...
    {
      title: zh("Copy last assistant message"),
      value: "messages.copy",
      category: "Session",
      run: () => {
        ...
        if (!lastAssistantMessage) {
          toast.show({ message: zh("No assistant messages found"), variant: "error" })
          dialog.clear()
          return
        }
        ...
        if (textParts.length === 0) {
          toast.show({ message: zh("No text parts found in last assistant message"), variant: "error" })
          dialog.clear()
          return
        }
        ...
        if (!text) {
          toast.show({
            message: zh("No text content found in last assistant message"),
            variant: "error",
          })
          dialog.clear()
          return
        ...
        clipboard
          .write?.(text)
          .then(() => toast.show({ message: zh("Message copied to clipboard!"), variant: "success" }))
          .catch(() => toast.show({ message: zh("Failed to copy to clipboard"), variant: "error" }))
        dialog.clear()
      },
    },
    {
      title: zh("Copy session transcript"),
      value: "session.copy",
      category: "Session",
      slash: {
        name: "copy",
      ...
      run: async () => {
        try {
          ...
          toast.show({ message: zh("Session transcript copied to clipboard!"), variant: "success" })
        } catch {
          toast.show({ message: zh("Failed to copy session transcript"), variant: "error" })
        }
        dialog.clear()
      },
    },
    {
      title: zh("Export session transcript"),
      ...
      run: async () => {
        try {
          ...
          if (options.openWithoutSaving) {
            ...
            toast.show({ message: zh("Session exported to") + ` ${filename}`, variant: "success" })
          }
        } catch {
          toast.show({ message: zh("Failed to export session"), variant: "error" })
        }
        dialog.clear()
      },
    },
    {
      title: zh("Background subagents"),
    ...
    {
      title: zh("Previous child session"),
                        ...
                        {(function () {
                          ...
                          const handleUnrevert = async () => {
                            const confirmed = await DialogConfirm.show(
                              dialog,
                              zh("Confirm Redo"),
                              zh("Are you sure you want to restore the reverted messages?"),
                            )
                            if (confirmed) {
                          ...
                          return (
                            ...
                            >
                              ...
                              >
                                <text fg={theme.textMuted}>{revert()!.reverted.length} {zh("message reverted")}</text>
                                <text fg={theme.textMuted}>
                                  <span style={{ fg: theme.text }}>{redoShortcut()}</span> {zh("to restore")}
                                </text>
                    ...
                    return (
                      <text fg={theme.text}>
                        <span style={{ bg: theme.secondary, fg: theme.background }}>
                          {directory ? zh("Directory") : zh("File")}
                        </span>
    ...
    <>
      ...
      <Show when={props.parts.some((x) => x.type === "tool" && x.tool === "task")}>
        <box paddingTop={1} paddingLeft={3}>
          <text fg={theme.text}>
            {childShortcut()}
            <span style={{ fg: theme.textMuted }}> {zh("view subagents")}</span>
            <Show
              when={
                sync.data.capabilities.experimentalBackgroundSubagents &&
                props.parts.some(
            ...
            >
              ...
              {backgroundShortcut()}
              <span style={{ fg: theme.textMuted }}> {zh("background")}</span>
            </Show>
          </text>
        </box>
      </Show>
          ...
          <box ref={(el: BoxRenderable) => alwaysSeparate.add(el)} paddingLeft={3}>
            <text marginTop={1}>
              ...
              <Show when={props.message.error?.name === "MessageAbortedError"}>
                <span style={{ fg: theme.textMuted }}> · {zh("interrupted")}</span>
              </Show>
            </text>
          </box>
        </Match>
    ...
    <Switch>
      <Match when={!props.done}>
        <box flexDirection="row">
          <Spinner color={fg()}>{props.title ? zh("Thinking") + ": " + props.title : zh("Thinking")}</Spinner>
        </box>
      </Match>
      <Match when={true}>
...
function GenericTool(props: ToolProps) {
  ...
  return (
    <Show
      ...
      fallback={
        <InlineTool icon="⚙" pending={zh("Writing command...")} complete={true} part={props.part}>
      ...
      >
        <box gap={1}>
          ...
          <Show when={collapsed().overflow}>
              <text fg={theme.textMuted}>{expanded() ? zh("Click to collapse") : zh("Click to expand")}</text>
          </Show>
        </box>
      </BlockTool>
    </Show>
...
function Shell(props: ToolProps) {
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool icon="$" pending={zh("Writing command...")} complete={stringValue(props.input.command)} part={props.part}>
...
function Glob(props: ToolProps) {
  const pathFormatter = usePathFormatter()
  return (
    <InlineTool icon="✱" pending={zh("Finding files...")} complete={stringValue(props.input.pattern)} part={props.part}>
...
function Grep(props: ToolProps) {
  const pathFormatter = usePathFormatter()
  return (
    <InlineTool icon="✱" pending={zh("Searching content...")} complete={stringValue(props.input.pattern)} part={props.part}>
...
function WebFetch(props: ToolProps) {
  return (
    <InlineTool icon="%" pending={zh("Fetching from the web...")} complete={stringValue(props.input.url)} part={props.part}>
...
function WebSearch(props: ToolProps) {
  return (
    <InlineTool icon="◈" pending={zh("Searching web...")} complete={stringValue(props.input.query)} part={props.part}>
...
function Edit(props: ToolProps) {
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool icon="←" pending={zh("Preparing edit...")} complete={stringValue(props.input.filePath)} part={props.part}>
...
function ApplyPatch(props: ToolProps) {
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool icon="%" pending={zh("Preparing patch...")} failure={zh("Patch failed")} complete={false} part={props.part}>
          Patch
        </InlineTool>
      </Match>
    </Switch>
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool
          icon="⚙"
          pending={zh("Updating todos...")}
          failure={zh("Todo update failed")}
          complete={false}
          part={props.part}
        >
          Updating todos...
...
function Question(props: ToolProps) {
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool icon="→" pending={zh("Asking questions...")} complete={count()} part={props.part}>
...
function Skill(props: ToolProps) {
  return (
    <InlineTool icon="→" pending={zh("Loading skill...")} complete={stringValue(props.input.name)} part={props.part}>
