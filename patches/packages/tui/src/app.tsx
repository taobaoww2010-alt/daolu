...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
    ...
    batch(() => {
      ...
      if (args.model) {
        ...
        if (!providerID || !modelID)
          return toast.show({
            variant: "warning",
            message: `${zh("Invalid model format")}: ${args.model}`,
            duration: 3000,
          })
      ...
      if (args.fork) {
        void sdk.client.session.fork({ sessionID: match }).then((result) => {
          if (result.data?.id) {
            ...
            toast.show({ message: zh("Failed to fork session"), variant: "error" })
          }
        })
      } else {
    ...
    [
      ...
      },
      {
        ...
        run: async () => {
          ...
          await clipboard
            ...
            .then(() => toast.show({ message: zh("Copied worktree path"), variant: "info" }))
            .catch(toast.error)
          dialog.clear()
        },
      },
      ...
      },
      {
        ...
        run: () => {
          if (local.model.variant.list().length === 0) {
            return toast.show({
              title: zh("No variants available"),
              message: zh("The current model does not support any variants."),
              variant: "info",
            })
          }
      ...
      },
      {
        ...
        run: async () => {
          ...
          toast.show({
            variant: "info",
            message: `${zh("Heap snapshot written to")} ${files?.join(", ")}`,
            duration: 5000,
          })
          dialog.clear()
        },
  ...
  event.on("session.deleted", (evt) => {
    if (route.data.type === "session" && route.data.sessionID === evt.properties.info.id) {
      ...
      toast.show({
        variant: "info",
        message: zh("The current session was deleted"),
      })
    }
  })
