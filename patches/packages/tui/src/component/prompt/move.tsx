...
export function usePromptMove(input: { projectID: () => string | undefined; sessionID: () => string | undefined }) {
  ...
  async function create(context?: string) {
    ...
    try {
      ...
      setProgress(zh("Creating session"))
      return directory
    } catch (err) {
      homeDestination?.clear()
      setProgress(undefined)
      setCreating(false)
      toast.show({ title: zh("Creating workspace failed"), message: errorMessage(err), variant: "error" })
      return
    }
  }
  ...
  function startSubmit() {
    if (progress()) setProgress(zh("Submitting prompt"))
  }
