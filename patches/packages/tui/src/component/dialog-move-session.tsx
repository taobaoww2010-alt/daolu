...
export function DialogMoveSession(props: DialogMoveSessionProps) {
  ...
  const options = createMemo<DialogSelectOption<MoveSessionSelection | undefined>[]>(() => {
    ...
    if (directories.loading && !data && !current) return [{ title: zh("Loading project directories..."), value: undefined }]
    const roots = [...(data ?? [])]
    ...
    if (roots.length === 0) return [{ title: zh("No project directories found"), value: undefined }]
    ...
    if (result.error) {
      ...
      if ("data" in result.error && result.error.data.forceRequired) {
        ...
        const choice = await DialogWorkspaceFileChanges.show(dialog, status?.data ?? [], {
          title: zh("Delete working copy?"),
          message: zh("This working copy has file changes. Do you want to delete it anyway?"),
        })
        if (choice !== "yes") {
          reopen()
          return
        ...
        if (forced.error) {
          toast.show({
            variant: "error",
            title: zh("Failed to delete project copy"),
          ...
          showError() ? (
            <box paddingLeft={4} paddingRight={4}>
              <text fg={theme.error} attributes={TextAttributes.BOLD}>
                {zh("Could not load project directories")}
              </text>
