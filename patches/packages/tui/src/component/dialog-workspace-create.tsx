...
async function loadWorkspaceAdapters(input: {
  ...
  try {
    ...
    input.toast.show({
      title: zh("Failed to load workspace adapters"),
...
export async function warpWorkspaceSession(input: {
  ...
  try {
    ...
    input.toast.show({
      title: zh("Failed to warp session"),
      message: errorMessage(err),
      variant: "error",
    })
    return false
  ...
  if (!result?.data) {
    if (result?.error && "name" in result.error && result.error.name === "VcsApplyError") {
      await DialogAlert.show(
        input.dialog,
        zh("Unable to Warp Session"),
        zh("Unable to apply file changes to this workspace. It has existing changes that conflict or is based off a different branch. Session has not been warped."),
      )
      return false
    }
  ...
  const options = createMemo<DialogSelectOption<WorkspaceSelectValue>[]>(() => {
    ...
    return [
      ...list.map((adapter) => ({
        ...
        category: zh("New workspace"),
      })),
      {
        title: zh("None"),
        value: { type: "none" as const },
        description: zh("Use the local project"),
        category: zh("Choose workspace"),
      },
      ...
      ...(hasMore
        ? [
            {
              title: zh("View all workspaces"),
              value: { type: "existing-list" as const },
              description: zh("Choose from all workspaces"),
        category: zh("Choose workspace"),
            },
          ]
        : []),
