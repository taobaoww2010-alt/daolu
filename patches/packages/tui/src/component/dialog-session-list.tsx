    ...
    const warp = async (selection: WorkspaceSelection) => {
      const workspaceID = await (async () => {
        ...
        try {
          ...
          toast.show({
            title: zh("Failed to create workspace"),
      ...
      <DialogSessionDeleteFailed
        ...
        onDelete={async () => {
          ...
          if (result.error) {
            toast.show({
              variant: "error",
              title: zh("Failed to delete workspace"),
  ...
  const options = createMemo(() => {
    ...
    const remaining = displayOrder
      ...
      .map((id) => {
        ...
        return buildOption(id, label === today ? zh("Today") : label)
      })
      .filter((x) => x !== undefined)

    return [...pinned.map((id) => buildOption(id, zh("Pinned"))).filter((x) => x !== undefined), ...remaining]
  })
              ...
              try {
                ...
                if (result.error) {
                  if (session?.workspaceID) {
                    ...
                    toast.show({
                      variant: "error",
                      title: zh("Failed to delete session"),
                      message: errorMessage(result.error),
                    })
                  }
                  setToDelete(undefined)
