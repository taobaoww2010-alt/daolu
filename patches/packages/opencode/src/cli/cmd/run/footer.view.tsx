...
export function RunFooterView(props: RunFooterViewProps) {
  ...
  const statusText = createMemo(() => {
    ...
    if (busy()) {
      return armed() ? zh("again to interrupt") : zh("interrupt")
    }
  ...
  const contextHints = createMemo(() => {
    ...
    if (foregroundSubagents() && backgroundShortcut()) {
      items.push({ kind: "background", key: backgroundShortcut(), label: zh("background") })
    }
    if (queuedPrompts().length > 0 && queuedShortcut()) {
      items.push({ kind: "queued", key: queuedShortcut(), label: `${queue()} ${zh("queued")}` })
    }
    if (activeTabs().length > 0 && subagentShortcut()) {
      items.push({ kind: "subagents", key: subagentShortcut(), label: zh("subagents") })
    }
  ...
  const commandHint = createMemo(() => {
    ...
    if (shell()) {
      return { key: "esc", label: zh("normal") }
    }

    if (command()) {
      return { key: command(), label: zh("cmd") }
    }
  })
    ...
    commands: [
      {
        ...
        title: zh("Open command palette"),
        category: "Prompt",
        run: openCommand,
      },
      {
        name: "variant.cycle",
        title: zh("Cycle model variant"),
        category: "Model",
        run: props.onCycle,
      },
    ],
    ...
    commands: [
      {
        ...
        title: zh("Manage queued prompts"),
        category: "Session",
        run: openQueuedMenu,
      },
    ],
