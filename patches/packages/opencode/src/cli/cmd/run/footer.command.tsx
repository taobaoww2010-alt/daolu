...
import { zh } from "@opencode-ai/tui/util/zh"
...
export function RunCommandMenuBody(props: {
  ...
  const entries = createMemo<CommandEntry[]>(() => {
        ...
        ? [
            {
              ...
              category: "Session",
              display: zh("View subagents"),
              footer:
                activeSubagentCount() > 0 ? `${activeSubagentCount()} ${zh("active")}` : `${props.subagents().length} ${zh("recent")}`,
              keywords: props
                .subagents()
        ...
        ? [
            {
              ...
              category: "Agent",
              display: zh("Manage queued prompts"),
              footer: `${props.queued().length} ${zh("queued")}`,
              keywords: props
                .queued()
        ...
        ? [
            {
              ...
              category: "Agent",
              display: zh("Switch model variant"),
    ...
    return [
      ...
      ...agent,
      ...commands,
      { action: "exit", category: "System", display: zh("Exit"), footer: "/exit", keywords: "/exit exit" },
    ]
  })
  ...
  return (
    ...
    >
      <RunFooterMenu
        ...
        rows={() => PANEL_LIST_ROWS}
        limit={PANEL_LIST_ROWS}
        empty={props.commands() ? zh("No skills found") : zh("Skills loading")}
        border={false}
        paddingLeft={PANEL_PAD}
        paddingRight={PANEL_PAD}
        grouped={false}
  ...
  const entries = createMemo<VariantEntry[]>(() => [
    {
      category: "",
      display: zh("Default"),
      description: props.current() === undefined ? zh("current") : undefined,
      keywords: "default",
      variant: undefined,
    ...
    ...props.variants().map((variant) => ({
      category: "",
      display: variant,
      description: props.current() === variant ? zh("current") : undefined,
      keywords: variant,
      variant,
  ...
  return (
    ...
    >
      <RunFooterMenu
        ...
        rows={() => PANEL_LIST_ROWS}
        limit={PANEL_LIST_ROWS}
        empty={props.providers() ? zh("No results found") : zh("Models loading")}
        border={false}
        paddingLeft={PANEL_PAD}
        paddingRight={PANEL_PAD}
        grouped={!query().trim()}
