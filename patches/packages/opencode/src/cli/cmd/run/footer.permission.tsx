...
export function RejectField(props: {
  ...
  return (
    <textarea
      ...
      maxHeight={3}
      wrapMode="word"
      placeholder={zh("Tell OpenCode what to do differently")}
...
export function RunPermissionBody(props: {
  ...
  const title = createMemo(() => {
    if (state().stage === "always") {
      return zh("Always allow")
    }

    if (state().stage === "reject") {
      return zh("Reject permission")
    }

    return zh("Permission required")
  })
      ...
      >
        ...
        <Switch>
          ...
          <Match when={state().stage === "reject"}>
            <box paddingLeft={1}>
              <text fg={props.theme.muted}>{zh("Tell OpenCode what to do differently")}</text>
            </box>
          </Match>
        </Switch>
      </box>
              ...
              <Show
                ...
                fallback={
                  <text fg={props.theme.muted} wrapMode="word" flexShrink={0}>
                    {zh("Waiting for permission event...")}
                  </text>
                }
              >
              ...
              >
                <box width="100%" flexDirection="column" gap={1}>
                  ...
                  <Show when={!info().diff && info().lines.length === 0}>
                    <box paddingLeft={1}>
                      <text fg={props.theme.muted}>{zh("No diff provided")}</text>
                    </box>
                  </Show>
                </box>
              </scrollbox>
        ...
        >
          ...
          >
            <box flexDirection="row" gap={2} flexShrink={0}>
              ...
              <text fg={props.theme.text}>
                enter <span style={{ fg: props.theme.muted }}>{zh("confirm")}</span>
              </text>
              <text fg={props.theme.text}>
