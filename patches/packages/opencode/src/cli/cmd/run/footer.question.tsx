      ...
      >
        <Show when={!single()}>
          <box flexDirection="row" gap={1} paddingLeft={1} flexShrink={0}>
            ...
            >
              <text fg={confirm() ? props.theme.surface : props.theme.muted}>{zh("Confirm")}</text>
            </box>
          </box>
        </Show>
            ...
            <box width="100%" flexGrow={1} flexShrink={1} paddingLeft={1}>
              ...
              >
                <box width="100%" flexDirection="column" gap={1}>
                  <box paddingLeft={1}>
                    <text fg={props.theme.text}>{zh("Review")}</text>
                  </box>
                  <For each={props.request.questions}>
                    {(item, index) => {
                        ...
                        <box paddingLeft={1}>
                          <text wrapMode="word">
                            ...
                            <span style={{ fg: answered() ? props.theme.text : props.theme.error }}>
                              {answered() ? value() : zh("(not answered)")}
                            </span>
                          </text>
                        </box>
                      )
          ...
          <box width="100%" flexGrow={1} flexShrink={1} paddingLeft={1} gap={1}>
            <box>
              <text fg={props.theme.text} wrapMode="word">
                {info()?.question}
                {info()?.multiple ? ` ${zh("(select all that apply)")}` : ""}
              </text>
            </box>
                    ...
                    >
                      <box flexDirection="row">
                        ...
                        <box backgroundColor={other() ? props.theme.line : undefined}>
                          ...
                          >
                            {info()?.multiple
                              ? `[${picked() ? "✓" : " "}] ${zh("Type your own answer")}`
                              : zh("Type your own answer")}
                          </text>
                        </box>
                        <Show when={!info()?.multiple}>
                      ...
                      >
                        <box paddingLeft={3}>
                          <textarea
                            ...
                            maxHeight={4}
                            wrapMode="word"
                            placeholder={zh("Type your own answer")}
              ...
              fallback={
                <>
                  <text fg={props.theme.text}>
                    enter <span style={{ fg: props.theme.muted }}>{zh("save")}</span>
                  </text>
                  <text fg={props.theme.text}>
                    esc <span style={{ fg: props.theme.muted }}>{zh("cancel")}</span>
                  </text>
                </>
              }
            >
              <Show when={!single()}>
                <text fg={props.theme.text}>
                  {"⇆"} <span style={{ fg: props.theme.muted }}>{zh("tab")}</span>
                </text>
              </Show>
              <Show when={!confirm()}>
                <text fg={props.theme.text}>
                  {"↑↓"} <span style={{ fg: props.theme.muted }}>{zh("select")}</span>
                </text>
              </Show>
              <text fg={props.theme.text}>
              ...
              <text fg={props.theme.text}>
                esc <span style={{ fg: props.theme.muted }}>{zh("dismiss")}</span>
              </text>
            </Show>
          </box>
        </Show>
