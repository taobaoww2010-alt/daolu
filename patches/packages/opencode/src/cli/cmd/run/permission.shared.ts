...
export function permissionInfo(request: PermissionRequest): PermissionInfo {
  ...
  if (request.permission === "doom_loop") {
    return {
      ...
      lines: [zh("This keeps the session running despite repeated failures.")],
    }
  }
  ...
  return {
    ...
    lines: [`${zh("Tool:")} ${request.permission}`],
  }
}
...
export function permissionAlwaysLines(request: PermissionRequest): string[] {
  if (request.always.length === 1 && request.always[0] === "*") {
    ...
    return [
      zh("This will allow the following patterns until OpenCode is restarted."),
      ...request.always.map((item) => `- ${item}`),
    ]
}

export function permissionLabel(option: PermissionOption): string {
  if (option === "once") return zh("Allow once")
  if (option === "always") return zh("Allow always")
  if (option === "reject") return zh("Reject")
  if (option === "confirm") return zh("Confirm")
  return zh("Cancel")
}
